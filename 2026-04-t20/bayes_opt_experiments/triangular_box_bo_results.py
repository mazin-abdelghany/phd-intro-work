...
@app.cell
def _(pd):
    triang_box = pd.read_csv(filepath_or_buffer="/tf/2026-04-t20/bayes_opt_experiments/triangular_box_bo.csv")
...
@app.cell
def _(n_loops, np, triang_box):
    for k in range(50):
        start_index = k * n_loops
        stop_index = start_index + n_loops

        analysis_set = triang_box.iloc[start_index:stop_index, 6:11]
        execute = triang_box.iloc[start_index:stop_index, 11]
        print("######")
        print(f"Run {k+1}:")
        print("######")
...
def _(n_loops, np, triang_box):
    ...
    for ell in range(50):
        ...
        _analysis_set = triang_box.iloc[_start_index:_stop_index, 6:11]
...
@app.cell
def _(n_loops, np, target_alpha, target_power, triang_box):
    epsilon1 = 0.01
    epsilon2 = 0.01
...
@app.cell
def _(epsilon1, epsilon2, np, target_alpha, target_power, triang_box):
    all_within_e1 = (triang_box["alpha"] <= target_alpha + epsilon1) & (triang_box["power"] >= target_power - epsilon1)

    all_within_e2_alpha = (triang_box["alpha"]>=target_alpha-epsilon2) & (triang_box["alpha"]<=target_alpha+epsilon2)
    all_within_e2_power = (triang_box["power"]>=target_power-epsilon2) & (triang_box["power"]<=target_power+epsilon2)
...
@app.cell
def _(triang_box):
    triang_box.iloc[0:500, 6:11]['obj_func'][300]
    return


@app.cell
def _(best_indeces, n_loops, tri_alpha, tri_max_ess, tri_obj, triang_box):
...
@app.cell
def _(best_indeces, n_loops, np, num_analyses, plt, tri, triang_box):
    ...
    for _i, idx in enumerate(best_indeces):
        ...
        _ax.plot(analysis_labels, triang_box.iloc[idx+add_to_index, 1:4], color = "purple", alpha = 0.1)
...
def _(
    ...
    _ax.plot(_analysis_labels, triang_box.iloc[_idx+_add_to_index, 1:4], color = "purple")
    _ax.plot(_analysis_labels, 
             np.concatenate((triang_box.iloc[_idx+_add_to_index, 4:6], [triang_box.iloc[_idx+_add_to_index, 3]])),
             color = "purple")
...
def _(absolute_best, triang_box):
    triang_box.iloc[absolute_best, 6:11]
    return


@app.cell
def _(absolute_best, analysis_labels, np, num_analyses, plt, tri, triang_box):
    _fig, _ax = plt.subplots(figsize=(12,6))
    ...
    # plot the best bounds
    _ax.plot(_analysis_labels, triang_box.iloc[absolute_best, 1:4], color = "purple")
    _ax.plot(_analysis_labels, 
             np.concatenate((triang_box.iloc[absolute_best, 4:6], [triang_box.iloc[absolute_best, 3]])),
             color = "purple")
