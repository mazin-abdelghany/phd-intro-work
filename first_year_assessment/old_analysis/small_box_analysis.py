...
def _(pd):
    ...
    small_box_rand = pd.read_csv(
        filepath_or_buffer="/tf/experiments_rand_simann_bo/random_search_experiments/small_box.csv"
    )

    small_box_bo = pd.read_csv(
        filepath_or_buffer="/tf/experiments_rand_simann_bo/bayes_opt_experiments/small_box_bo.csv"
    )

    # importing the corrected data
    small_box_sim_ann = pd.read_csv(
        filepath_or_buffer=
        "/tf/experiments_rand_simann_bo/simulated_annealing_experiments/small_box_t100_results.csv"
    )
...
def _(np, plt, small_box_bo, small_box_rand, small_box_sim_ann):
    ...
    ax[0,0].text(0.95, 0.045, np.round(np.median(small_box_rand["alpha"]), decimals=2))
    ...
    ax[0,2].text(0.95, 0.045, np.round(np.median(small_box_bo["alpha"]), decimals=2))

    ax[1,0].violinplot(small_box_rand["power"], showextrema=False, showmedians=True)
    ax[1,0].text(0.955, 0.855, np.round(np.median(small_box_rand["power"]), decimals=2))

    ax[1,1].violinplot(small_box_sim_ann["power"], showextrema=False, showmedians=True)
    ax[1,1].text(0.955, 0.855, np.round(np.median(small_box_sim_ann["power"]), decimals=2))

    ax[1,2].violinplot(small_box_bo["power"], showextrema=False, showmedians=True)
    ax[1,2].text(0.955, 0.855, np.round(np.median(small_box_bo["power"]), decimals=2))

    ax[2,0].violinplot(small_box_rand["sample_size"], showextrema=False, showmedians=True)
    ax[2,0].text(0.97, 95, np.round(np.median(small_box_rand["sample_size"]).astype(int), decimals=0))

    ax[2,1].violinplot(small_box_sim_ann["sample_size"], showextrema=False, showmedians=True)
    ax[2,1].text(0.97, 99, np.round(np.median(small_box_sim_ann["sample_size"]).astype(int), decimals=0))

    ax[2,2].violinplot(small_box_bo["sample_size"], showextrema=False, showmedians=True)
    ax[2,2].text(0.97, 94, np.round(np.median(small_box_bo["sample_size"]).astype(int), decimals=0))

    ax[3,0].violinplot(small_box_rand["max_ess"], showextrema=False, showmedians=True)
    ax[3,0].text(0.97, 234, np.round(np.median(small_box_rand["max_ess"]).astype(int), decimals=0))

    ax[3,1].violinplot(small_box_sim_ann["max_ess"], showextrema=False, showmedians=True)
    ax[3,1].text(0.96, 243, np.round(np.median(small_box_sim_ann["max_ess"])).astype(int))

    ax[3,2].violinplot(small_box_bo["max_ess"], showextrema=False, showmedians=True)
    ax[3,2].text(0.96, 233, np.round(np.median(small_box_bo["max_ess"])).astype(int))
...
@app.cell
def _(n_loops, np, small_box_sim_ann):
    best_indices = []
    for ell in range(50):
        _start_index = ell * n_loops
        _stop_index = _start_index + n_loops

        _analysis_set = small_box_sim_ann.iloc[_start_index:_stop_index, 6:11]
...
@app.cell
def _(n_loops, np, pd, small_box_sim_ann, target_alpha, target_power):
    feasibility = {
        "run" : [],
        "feasibility" : [],
        "strict_feasibility" : [],
...
@app.cell
def _(n_loops, np, small_box_sim_ann, tri_alpha, tri_max_ess, tri_obj):
...
def _(
    ...
    _fig.suptitle("Small hyperrectangle: Best bounds", y=1.05)
...
def _(
    ...
    _fig.suptitle("Small hyperrectangle: Random run 1", y=1.05)
