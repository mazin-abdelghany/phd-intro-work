...
@app.cell
def _(pd):
    large_box = pd.read_csv(filepath_or_buffer="/tf/2026-04-t20/bayes_opt_experiments/large_box_bo.csv")
...
@app.cell
def _(large_box, n_loops, np):
    for k in range(50):
        start_index = k * n_loops
        stop_index = start_index + n_loops

        analysis_set = large_box.iloc[start_index:stop_index, 6:11]
        execute = large_box.iloc[start_index:stop_index, 11]
        print("######")
        print(f"Run {k+1}:")
        print("######")
...
def _(large_box, n_loops, np):
    ...
    for ell in range(50):
        ...
        _analysis_set = large_box.iloc[_start_index:_stop_index, 6:11]
...
@app.cell
def _(large_box, n_loops, np, target_alpha, target_power):
    epsilon1 = 0.01
    epsilon2 = 0.01
...
@app.cell
def _(epsilon1, epsilon2, large_box, np, target_alpha, target_power):
    all_within_e1 = (large_box["alpha"] <= target_alpha + epsilon1) & (large_box["power"] >= target_power - epsilon1)

    all_within_e2_alpha = (large_box["alpha"]>=target_alpha-epsilon2) & (large_box["alpha"]<=target_alpha+epsilon2)
    all_within_e2_power = (large_box["power"]>=target_power-epsilon2) & (large_box["power"]<=target_power+epsilon2)
...
@app.cell
def _(best_indeces, large_box, n_loops, tri_alpha, tri_max_ess, tri_obj):
...
@app.cell
def _(best_indeces, large_box, n_loops, np, num_analyses, plt, tri):
    ...
    for _i, idx in enumerate(best_indeces):
        ...
        _ax.plot(analysis_labels, large_box.iloc[idx+add_to_index, 1:4], color = "purple", alpha = 0.1)
        _ax.plot(analysis_labels, 
                 np.concatenate((large_box.iloc[idx+add_to_index, 4:6], [large_box.iloc[idx+add_to_index, 3]])),
...
def _(
    ...
    _ax.plot(_analysis_labels, large_box.iloc[_idx+_add_to_index, 1:4], color = "purple")
    _ax.plot(_analysis_labels, 
             np.concatenate((large_box.iloc[_idx+_add_to_index, 4:6], [large_box.iloc[_idx+_add_to_index, 3]])),
             color = "purple")

    _ax.text(2.75, -4, np.round(large_box.iloc[_idx+_add_to_index]['obj_func'], decimals = 4))
...
@app.cell
def _(large_box, np):
    absolute_best = np.argmin(large_box['obj_func'])
    return (absolute_best,)


@app.cell
def _(absolute_best, large_box):
    large_box.iloc[absolute_best, 6:11]
    return


@app.cell
def _(absolute_best, analysis_labels, large_box, np, num_analyses, plt, tri):
    _fig, _ax = plt.subplots(figsize=(12,6))
    ...
    # plot the best bounds
    _ax.plot(_analysis_labels, large_box.iloc[absolute_best, 1:4], color = "purple")
    _ax.plot(_analysis_labels, 
             np.concatenate((large_box.iloc[absolute_best, 4:6], [large_box.iloc[absolute_best, 3]])),
             color = "purple")

    _ax.text(2.75, -4, np.round(large_box.iloc[absolute_best]['obj_func'], decimals = 4))
