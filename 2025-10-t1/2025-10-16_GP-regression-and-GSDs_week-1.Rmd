---
title: "GSD and GP regression - week 1"
output: 
  html_document:
    toc: true
    toc_depth: 3
...
### MV normal probability calculations
...
$$
\int_{l_1}^{u_1}\int_{l_2}^{u_2}\int_{u_3}^{\infty} \phi_2 \left( 
\begin{bmatrix}
    y_1 & y_2 & y_3
\end{bmatrix}, 
\begin{bmatrix} 
...
Let's start by modeling a circle.
```{r}
x  <- matrix(c(seq(-1, -0.9, length.out = 10),
               seq(-0.9, 0.9, length.out = 20),
               seq(0.9, 1, length.out = 10)), ncol = 1)
y  <- sqrt(1 - x^2) 
...
gp <- gp_optim(gp_empty, x, y, verbose = FALSE)
```

```{r}
# compute the predictive mean and variance in a grid of points
xt   <- seq(-1.5, 1.5, len=150)
...
The Gaussian process does not want to create an asymptote at the critical values of -1 and 1.

We can model a "better behaved" function. We can start with a single point.
```{r}
n <- 1
...
Now 6, points closer together, with noise:
```{r}
n <- 6
x  <- matrix(seq(-1, 1, length.out = n), ncol = 1)
y  <- x^2 + rnorm(6, mean = 0, sd = 0.1)
...
Now 7, with the point $(10, 100)$:
```{r}
n <- 6
x  <- matrix(c(seq(-2, 2, length.out = n), 10), ncol = 1)
y  <- x^2
...
Now 7 with the point $(0, 0)$:
```{r}
n <- 6
x  <- matrix(c(seq(-2, 2, length.out = n), 0), ncol = 1)
y  <- x^2
...
Add some noise to the case of 7 points:
```{r}
n <- 7
x  <- matrix(seq(-2, 2, length.out = n), ncol = 1)
y  <- x^2 + rnorm(7, mean = 0, sd = 0.3)
