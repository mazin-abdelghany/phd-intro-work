...
//onestagesamplesize finds the sample size required for a one-stage trial with given design parameters
...
// changed return type to collect data
std::tuple<double, double, double, double> functionvalue_deltaminimax(vector<double>& candidateparameters,double delta0,double delta1,double sigma,[[maybe_unused]]double K,double requiredtypeIerror,double requiredtypeIIerror,double penaltyparameter,int numberrestarts)
{
 ...
 return{functionvalue, typeIerror, power, expectedsamplesize_dm}; 
}
...
void simulatedannealing_deltaminimax(double delta0,double delta1,double sigma,double K,double requiredtypeIerror,double requiredpower,vector<double> &initialparameters,vector<double> lowerranges,vector<double> upperranges,vector<double>& initialparameterssigma,double initialcosttemperature,double finalparametersigma,double finalcosttemperature,int numbercandidategenerationsperrestart,int minnumberrestarts,vector<double> &finalparameters,double *finalfunctionvalue,double penaltyparameter)
{
  ...
  // split up the variable declarations and function call to correct for returning tuple
  double x,numbersincereduction=0,currentfunctionvalue,reductioninfunctionvalue;
 auto [minimumfunctionvalue, init_alpha, init_power, init_ess_dm]=functionvalue_deltaminimax(initialparameters,delta0,delta1,sigma,K,requiredtypeIerror,(1-requiredpower),penaltyparameter,-2);
...
do
 ...
 auto [newfunctionvalue, new_alpha, new_power, new_ess_dm]=functionvalue_deltaminimax(candidateparameters,delta0,delta1,sigma,K,requiredtypeIerror,(1-requiredpower),penaltyparameter,numberrestarts-minnumberrestarts);

file << csv_index << ","
             << loop_n << ","
             << numberrestarts << ","
             << candidateparameters.at(2) << "," // upper1
             << candidateparameters.at(4) << "," // upper2
             << candidateparameters.at(6) << "," // upper3
             << candidateparameters.at(1) << "," // lower1
             << candidateparameters.at(3) << "," // lower2
             << new_alpha << ","
             << new_power << ","
             << candidateparameters.at(0) << "," // sample size
             << new_ess_dm << ","
             << newfunctionvalue<< ","
             << costtemperature << "\n";
...
auto [new_minimumfunctionvalue,min_alph,min_pow,min_ess_d]=functionvalue_deltaminimax(minimumparameters,delta0,delta1,sigma,K,requiredtypeIerror,(1-requiredpower),penaltyparameter,1);
 currentfunctionvalue=new_minimumfunctionvalue;
 currentparameters=minimumparameters;
...
do
 ...
 auto [newfunctionvalue, new_alpha, new_power, new_ess_dm]=functionvalue_deltaminimax(candidateparameters,delta0,delta1,sigma,K,requiredtypeIerror,(1-requiredpower),penaltyparameter,1);
...
auto [minimumfunctionvalue, min_alpha, min_power, min_ess_dm] =functionvalue_deltaminimax(minimumparameters,delta0,delta1,sigma,K,requiredtypeIerror,(1-requiredpower),penaltyparameter,numberrestarts-minnumberrestarts);
     previousrestart=minimumfunctionvalue;
