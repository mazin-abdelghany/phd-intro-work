...
5. Run `docker compose -f gpflow.yaml up`
    - Depending on your system, this command may take several minutes.
5. If the Docker container has been built before and the `gpflow.yaml` file has changed, instead run `docker compose -f gpflow.yaml --build`
     - Depending on your system, this command may take several minutes.
...
1. Run `docker exec -it bayes-opt-jupyter-nb-1 sh` OR `docker exec -it bayes-opt-marimo-nb-1 sh`
...
2. Run `pip install /tf/pyGroupSequentialDesigns`. Warnings can be ignored.
3. Output should contain `Successfully installed py_group_sequential_designs-0.0.1`
4. Imports in Jupyter or Marimo `from py_group_sequential_designs import . . . ` should run without issue.